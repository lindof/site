...
define([
    ...
    $.widget('mageworx.optionFeatures', {
        ...
        firstRun: function firstRun(optionConfig, productConfig, base, self) {
            ...
            $('.product-social-sharing').on('click', function (e) {
                var url,
                    type = $(this).data('type'),
                    productName = $(this).data('name'),
                    shareableLink = self.getShareableLink(base);

                e.preventDefault();

                switch (type) {
                    case 'facebook':
                        url = 'http://www.facebook.com/sharer.php?u=' + shareableLink;
                        break;
                    case 'pinterest':
                        url = 'http://pinterest.com/pin/create/button/?url=' + shareableLink;
                        break;
                    case 'twitter':
                        url = 'http://twitter.com/share?url=' + shareableLink +'&text=' + productName;
                        break;
                    case 'linkedin':
                        url = 'https://www.linkedin.com/shareArticle?mini=true&url=' + shareableLink + '&t=' + productName;
                        break;
                    default:
                        url = false;
                }
        ...
        getShareableLink: function getShareableLink(base) {
            ...
            var selectedConfigOptionString = this.getConfigurableSelectedOptionsString(base);
            if (selectedOptionsString) {
                shareableLink += '?config=';
                shareableLink += selectedOptionsString;
            }

            let concatSymbol = selectedOptionsString ? '&' : '?';

            if(selectedConfigOptionString) {
                shareableLink += concatSymbol;
                shareableLink += selectedConfigOptionString;
            }
         ...
         *
         */
        getConfigurableSelectedOptionsString: function getConfigurableSelectedOptionsString(base) {
            var selectedConfigOptionsString = '';
            var self = this;

            base.collectSelectedData();
            var selectedData = this.getConfigSelectedData();
            var selectedOptionsArray = [];
        ...
        getConfigSelectedData: function getConfigSelectedData() {
            if (_.isUndefined(window.cpSelectedData)) {
                window.cpSelectedData = {};
            }
            return window.cpSelectedData;
        }
    });
