    ...
    function ($, _, $t, priceUtils) {
        ...
        $.widget('mageworx.optionSwatches', {
            ...
            processSwatchLabel: function ($selectOption) {
                ...
                // Create a container inside the swatch options to render stock message
                if (optionLabel.parent().parent().find('span#stock-container').length <= 0) {
                    optionLabel.parent().parent().find('.swatch-attribute').after('<span id="stock-container"></span>');
                }
                var stockMessageText = [];
                ...
                if ($select.val()) {
                    $(selectOptions).each(function () {
                        if (_.contains($select.val(), $(this).attr('value')) || $select.val() === $(this).attr('value')) {
                            ...
                            /**
                             * Act360 Append a string to the selected label showing stock status
                             * Find custom attribute from Custom handler
                             * @see \Act\StockMessage\Plugin\Product\View\Options\Type\Select
                             */
                            var stockLabel = '';
                            var selectedLabel = $swatch.data('option-label');
                            var selectedSku = $swatch.data('option-sku');
                            var stockInfo = $(".act-mageworx-swatch-container[data-option-id='" + optionId + "'][data-option-label='" + selectedLabel + "']");
                            if (stockInfo) {
                                var totalStock = stockInfo.data('option-stock');
                                if (totalStock > 0 && totalStock <= 5) {
                                    stockLabel = "<span class='act-stock-message'>Hurry only a few left</span>"
                                }
                                if (totalStock == 1) {
                                    stockLabel = "<span class='act-stock-message'>Hurry only 1 remaining</span>"
                                }
                                // console.log("Stock", totalStock);
                            }
                            if(stockLabel!='') stockMessageText.push(stockLabel);
                        }
                    });
                }
                var $el = optionLabel.parent().find('span#value');
                var $stockEl = optionLabel.parent().parent().find('span#stock-container');
                if (isSelectedOptionExist === false) {
                    $el.html('');
                    $stockEl.html('');
                } else {
                    $el.html(' - ' + labelText.join(', '));
                    $stockEl.html(stockMessageText.join('<br/>'));
                }

                //Unbind stock message handle
                $(".act-stock-message").click(function (event) {
            ...
            _initEventListener: function () {
                ...
                // console.log(this.options.optionClass)
            },
            ...
            _onClick: function (option) {
                ...
                var limit = parseInt(select.attr('data-limit'));
                if(limit>0 && !$(option).hasClass('selected') && $('#select_' + optionId + ' option:selected').length>=limit) {
                    return;
                }
                var gallery = $('[data-gallery-role="gallery"]', '.column.main');
