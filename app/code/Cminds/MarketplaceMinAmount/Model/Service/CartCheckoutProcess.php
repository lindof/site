...
//            $customer = $this->customerSession->getCustomer();
            ...
            $customer = $this->customerFactory->create();
            $this->customerResource->load($customer, $this->customerSession->getCustomerId());

            $supplier = $customer;
            $limitValue = $this->getLimitValue($supplier);
            $limitQtyValue = $this->getLimitQtyValue($supplier);
            $per = $supplier->getSupplierMinOrderAmountPer();

            if ($per == \Cminds\MarketplaceMinAmount\Model\Config\Source\MinimumAmount::NONE) {
                return $this;
            }


            if ($per == \Cminds\MarketplaceMinAmount\Model\Config\Source\MinimumAmount::ORDER) {
                $status = 0;
            ...
            if ($per == \Cminds\MarketplaceMinAmount\Model\Config\Source\MinimumAmount::DAY) {
    ...
    public function getErrorResult($supplier, $limitValue, $amount, $limitQty, $qty, $status)
    {
        ...
        if ($status === 1) {
            $result['message'] = __(
                'Minimum Order Amount for products should be %1. Currently, you reached %2 in your Cart.',
            ...
            if ($status === 2) {
                $result['message'] = __(
                    'Minimum Order Qty for products should be %1. Currently, you reached %2 in your Cart.',
                    $this->dataHelper->convertMinOrderQty($limitQty),
                    $qty
                );
            } else {
                $result['message'] = __(
                    'Minimum Order Amount for products should be %1 and the minimum Order Qty should be %2. Currently, you reached %3 in the Amount and %4 in the Qty in your Cart.',
                    $this->pricingHelper->currency($limitValue, true, false),
                    $this->dataHelper->convertMinOrderQty($limitQty),
                    $this->pricingHelper->currency($amount, true, false),
                    $qty
