...
use Cminds\Marketplace\Model\ResourceModel\Methods as MarketplaceMethodsResource;
...
class Methodssave extends AbstractController
{
    ...
    /**
     * @var MarketplaceMethodsResource
     */
    private $marketplaceMethodsResource;
     ...
     * @param MarketplaceMethodsResource $marketplaceMethodsResource
    ...
    public function __construct(
        ...
        MarketplaceMethodsResource $marketplaceMethodsResource,
        ...
        $this->marketplaceMethodsResource = $marketplaceMethodsResource;
    ...
    public function execute()
    {
        ...
        try {
            ...
            foreach ($postData['id'] as $id => $data) {
                ...
                } elseif ($shippingMethod === "store_pickup") {
                    $shipping
                        ->setStorePickup(1)
                        ->setStoreAddress($postData['store_pickup_address'][$id]);
                }
            ...
            foreach ($data['id'] as $i => $k) {
                if (!in_array( $k, $removedItems )) {
                    ...
                    if ($shippingMethod === 'store_pickup') {
                        $address = $data['store_pickup_address'][$i];
                        if (trim($address) === '') {
                            $message = 'Store Address is empty in Store Pick Up Shipping method. Please fill out this field.';
