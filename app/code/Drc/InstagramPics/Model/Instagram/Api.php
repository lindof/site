...
namespace Drc\InstagramPics\Model\Instagram;
...
class Api
{
    ...
    {
        if (is_array($config)) {
            $this->setApiKey($this->_configHelper->getClientId());
            $this->setApiSecret($this->_configHelper->getClientSecret());
            $this->setApiCallback($this->_configHelper->getRedirectUrl());
        } elseif (is_string($config)) {
            $this->setAccessToken($config);
        } else {
            throw new \Magento\Framework\Exception\LocalizedException('Error: __construct() - Configuration data is missing.');
    ...
    public function modifyRelationship($action, $user)
    {
        if (in_array($action, $this->_actions) && isset($user)) {
            return $this->_makeCall('users/' . $user . '/relationship', ['action' => $action], 'POST');
        }

        throw new \Magento\Framework\Exception(
            'Error: modifyRelationship() | This method requires an action command and the target user id.'
        );
    }
    ...
    {
        return $this->_makeCall('tags/search', ['q' => $name]);
    }
    ...
    {
        return $this->_makeCall('media/' . $id . '/comments', ['text' => $text], 'POST');
    }
    ...
    public function pagination($obj, $limit = 0)
    {
        if (is_object($obj) && !is_null($obj->pagination)) {
            ...
            if (isset($obj->pagination->next_max_tag_id)) {
                return $this->_makeCall($function, ['max_tag_id' => $obj->pagination->next_max_tag_id, 'count' => $count]);
            }

            return $this->_makeCall($function, ['next_max_id' => $obj->pagination->next_max_id, 'count' => $count]);
        }
        throw new \Magento\Framework\Exception("Error: pagination() | This method doesn't support pagination.");
    }
    ...
    public function getOAuthToken($code, $token = false)
    {
        ...
        /*echo $this->getApiKey().'</br>';
		echo $this->getApiSecret().'</br>';
		echo $this->getApiCallback().'</br>';
		echo $code.'</br>';
		die();*/
    ...
    {
        //$this->_accesstoken = '3960358376.c8f4461.11ec4f4e83ec4f2bb84e645327aa175c';
        $this->_accesstoken = $this->_configHelper->getAccessToken();
    
        if (!isset($this->_accesstoken)) {
            throw new \Magento\Framework\Exception("Error: _makeCall() | $function - This method requires an authenticated users access token.");
        ...
        if (!$jsonData) {
            throw new \Magento\Framework\Exception('Error: _makeCall() - cURL error: ' . curl_error($ch));
        }
    ...
    private function _makeOAuthCall($apiData)
    {
        ...
        if (!$jsonData) {
            throw new \Magento\Framework\Exception('Error: _makeOAuthCall() - cURL error: ' . curl_error($ch));
        }
