...
class CustomOptions extends OriginalCustomOptions implements ModifierInterface
{
    ...
    public function modifyData(array $data)
    {
        ...
        foreach ($productOptions as $index => $option) {
            ...
            foreach ($values as $value) {
                ...
                if ($value->getLayer()) {
                    $imageUrl = $this->helper->getBaseUrl() . $this->helper->getBasePath(). $value->getLayer();
    ...
    {
        $columns = &$this->meta[static::GROUP_CUSTOM_OPTIONS_NAME]['children']
            [static::GRID_OPTIONS_NAME]['children']['record']['children']
            [static::CONTAINER_OPTION]['children']
            [static::GRID_TYPE_SELECT_NAME]['children']['record']['children'];
            ...
            'arguments' => [
                'data' => [
                    'config' => [
                        ...
                        'sortOrder'     => $sortOrder,
                        'content'       => __('Please make sure that the images you upload for Image Layer are of '
                            . 'the same exact size as the product image itself, '
                            . 'otherwise they won\'t be correctly positioned!'),
                    ],
                ],
            ],
        ];
