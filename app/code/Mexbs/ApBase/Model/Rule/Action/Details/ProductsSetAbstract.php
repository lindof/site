...
abstract class ProductsSetAbstract extends \Mexbs\ApBase\Model\Rule\Action\Details\Condition\Product\Combine{
    ...
    protected function getMatchedQtyOfItem($item){
        ...
        return $itemQtyUsedForThisPromo;
    }
	...
	public function hasAddressConditionsInActionInAnyGroup(){
        foreach($this->_getValidSetPartIndexes() as $setPartIndex){
            $setPartActionDetails = $this->getData('set_part'.$setPartIndex.'_action_details')->getActionDetails();
            $setPartActionDetail = $setPartActionDetails[0];
            if($setPartActionDetail->hasAddressConditionsInAction()){
                return true;
            }
        }
        return false;
    ...
    protected function _calcCleanAndFullSets($priceToItem, $validRuleSetPartsIndexes){
        ...
        foreach($validRuleSetPartsIndexes as $setPartIndex){
            ...
            foreach($priceToItem as $item){
				$qtyMatchedInOtherRules = $this->apHelper->getItemQtyMatchedInOtherRules($item, $this->getRule()->getId());
                $qtyLeftToMatchForItem = max(0, $item->getQty() - $qtyMatchedInOtherRules);
                if($qtyLeftToMatchForItem == 0){
                    continue;
                }
				
                if($this->apHelper->validateActionDetail($setPartActionDetail, $item)){
                    $validPriceToItem[] = [
                      'matched_qty' => $qtyMatchedInOtherRules,
                      'item' => $item
                    ];
                }
            }
            ...
            $item = $validPriceToItem[$currentItemKey]['item'];
            $qtyMatchedInOtherRules = $validPriceToItem[$currentItemKey]['matched_qty'];
           ...
           while($currentKeyIndex < count($validPriceToItemKeys)){
                $matchedQty = $qtyMatchedInOtherRules;
